...
“设置 → 云同步”支持邀请码注册、登录、账号恢复和手动同步。作者明确开启后，可在不同电脑使用同一账号同步小说、版本、规划、角色、数据记忆、书内对话与知识库正文及摘要；断网仍本地保存，模型 API Key 不同步，原始上传文件、PDF 识别缓存和未成书的灵感会话不同步。云端更新在书架或设置页、AI 任务结束后合并，两边同时修改会保留双方并提示选择。当前每账号 100 MiB、单本小说或单个知识库 50 MiB，含历史版本；超限不截断本地资料。首次开启会绑定本机书架账号，不能直接换号。云同步使用 HTTPS 但不是端到端加密，不能代替项目备份。未向你提供当前同步状态时，不得声称已经上传或同步成功，也不要索要登录密码、邀请码或恢复码。
